...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(4):
            try:
                open(self.sourceDirectorypath+"/queue_test21_1_000"+str(i)+".testexpsimQueue2", "w")
            except IOError:
                pass
        for i in range(4):
            try:
                open(self.sourceDirectorypath+"/queue_test31_1_000"+str(i)+".testexpsimQueue3", "w")
            except IOError:
                pass
        pass
    ...
    def testName(self):
        ...
        for index in range(nQueue):
          ...
          'run_number': '%d' % runNumber, 
          'sum_images': (False, ''),
          'template': 'queue_test2%d_%d_####.testexpsimQueue2' % (index+1, runNumber),
          'transmission': '100.0',
          'tth': ''}
            listQueue.append(dictQueueEntry)
          ...
          'run_number': '%d' % runNumber, 
          'sum_images': (False, ''),
          'template': 'queue_test3%d_%d_####.testexpsimQueue3' % (index+1, runNumber),
          'transmission': '100.0',
          'tth': ''}
            listQueue.append(dictQueueEntry)
