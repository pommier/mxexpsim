...
@Device
public class SaveMxSim {

    private static final Logger logger = LoggerFactory.getLogger(SaveMxSim.class);
    private static final XLogger xlogger = XLoggerFactory.getXLogger(SaveMxSim.class);
	//========================================================
	...
	*/
	...
	public Boolean generateImages(String generateImagesIn) throws DevFailed {
		xlogger.entry();
		...
		try {
			...
			int i=0;
		/*	while (i<6){
				this.setgenerateImageStatut(false);
				try {
					Thread.sleep(1000);
			...
			this.setgenerateImageStatut(true);*/
			/*
			ImageGenerator generator = new ImageGenerator(Integer.parseInt((String) jsonObj.get("first_image")),Integer.parseInt((String) jsonObj.get("number_images")),jsonObj.get("template")+config.getSuffix(), config.getSourceImageDirectory(),(String) jsonObj.get("process_directory"));
	...
	/*----- PROTECTED REGION ID(MxSim.methods) ENABLED START -----*/
	 private static void startNoDB() throws DevFailed {
			...
			ServerManager.getInstance().addClass(SaveMxSim.class.getCanonicalName(), SaveMxSim.class);
			ServerManager.getInstance().startError(new String[] { "1", "-nodb", "-dlist", "tmp/test/device", },
				SaveMxSim.class.getSimpleName());
		    }
